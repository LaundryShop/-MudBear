...
// シェーダ名(ディレクトリも含む)
const char SHADER_NAME[] = "MeshSkin.hlsl";
...
// Xファイルからメッシュを読み込む.
HRESULT D3DXPARSER::LoadMeshFromX( LPDIRECT3DDEVICE9 pDevice9, const char* FileName )
{
...
bool D3DXPARSER::GetMatrixFromBone(const char* sBoneName, D3DXMATRIX* pOutMat )
{
	LPD3DXFRAME pFrame;
...
bool D3DXPARSER::GetPosFromBone(const char* sBoneName, D3DXVECTOR3* pOutPos )
{
	D3DXMATRIX mBone;
...
HRESULT clsD3DXSKINMESH::CreateFromX( const char* szFileName )
{
...
void clsD3DXSKINMESH::DrawPartsMesh( SKIN_PARTS_MESH* pMesh, D3DXMATRIX World, MYMESHCONTAINER* pContainer )
{
	...
	D3DXMatrixRotationY( &Yaw,	m_vRot.y );		// Y軸回転.
	D3DXMatrixRotationX( &Pitch,m_vRot.x );		// X軸回転.
	D3DXMatrixRotationZ( &Roll, m_vRot.z);		// Z軸回転.
	// 回転行列(合成)
	...
	for( DWORD i=0; i<pMesh->dwNumMaterial; i++ )
	{
		...
		//	吉田追加（テクスチャ切り替え）くまさん.
		//-----------------------
		if (pMesh->dwNumMaterial == 1) {
			m_pDeviceContext->PSSetSamplers(0, 1, &m_pSampleLinear);
			if (m_IsEnabelChangeTexttureSystem == 1) {
				int str_legth = lstrlen(pMesh->pMaterial[i].szTextureName);
				int str_no = 0;
				bool change_flag = false;
				for (str_no = str_legth; 0 < str_no; str_no--) {
					if (pMesh->pMaterial[i].szTextureName[str_no] == '\\') {
						str_no++;
						change_flag = true;
						break;
					}
				...
				if (change_flag == true)
				{
					//対象のテクスチャ名なのか判定.
					if (strcmp(
						&pMesh->pMaterial[i].szTextureName[str_no],
						m_pChangeTextures->szTargetTextureName) == 0)
					{
						//別のテクスチャを設定.
						m_pDeviceContext->PSSetShaderResources(0, 1, &m_pChangeTextures->pTexture);
			...
			else if (m_IsEnabelChangeTexttureSystem == 2) {
				//別のテクスチャを設定二つ目.
				m_pDeviceContext->PSSetShaderResources(0, 1, &m_pChangeTextures->pTextureTest);
			}
			else {
				//デフォルトテクスチャを設定.
				m_pDeviceContext->PSSetShaderResources(0, 1, &pMesh->pMaterial[i].pTexture);
		...
		else
		{
			...
			//	ボスくまさん用.
			m_pDeviceContext->PSSetSamplers(0, 1, &m_pSampleLinear);
			if (m_B_IChangeTexttureSystem[i] == 1) {
				int str_legth = lstrlen(pMesh->pMaterial[i].szTextureName);
				int str_no = 0;
				bool change_flag = false;
				for (str_no = str_legth; 0 < str_no; str_no--) {
				...
				if (change_flag == true)
				{
					...
					if (strcmp(
						&pMesh->pMaterial[i].szTextureName[str_no],
						m_pBChangeTextures[i]->szTargetTextureName) == 0)
					{
						//別のテクスチャを設定.
						m_pDeviceContext->PSSetShaderResources(0, 1, &m_pBChangeTextures[i]->pTexture);
			...
			else if (m_B_IChangeTexttureSystem[i] == 2) {
				//別のテクスチャを設定二つ目.
				m_pDeviceContext->PSSetShaderResources(0, 1, &m_pBChangeTextures[i]->pTextureTest);
			}
			else {
				//デフォルトテクスチャを設定.
				m_pDeviceContext->PSSetShaderResources(0, 1, &pMesh->pMaterial[i].pTexture);
...
HRESULT clsD3DXSKINMESH::Release()
{
	...
	for (int i = TEX_NO; i >= 0; i--)
	{
		m_pBChangeTextures[i] = nullptr;
	}
	m_pChangeTextures = nullptr;
	//SAFE_DELETE(m_pChangeTextures);
...
// 指定したボーン情報(行列)を取得する関数.
bool clsD3DXSKINMESH::GetMatrixFromBone( const char* sBoneName, D3DXMATRIX* pOutMat )
{
	if( m_pD3dxMesh != NULL ){
...
// 指定したボーン情報(座標)を取得する関数.
bool clsD3DXSKINMESH::GetPosFromBone( const char* sBoneName, D3DXVECTOR3* pOutPos )
{
	if( m_pD3dxMesh != NULL ){
		D3DXVECTOR3 tmpPos;
...
//-----------------------
//吉田.
HRESULT clsD3DXSKINMESH::InitBlend()
{
...
//透過設定の切り替え.
void clsD3DXSKINMESH::SetBlend(bool EnableAlpha)
{
	//ﾌﾞﾚﾝﾄﾞｽﾃｰﾄの設定.
