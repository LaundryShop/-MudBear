...
//初期化.
HRESULT CDX9SkinMesh::Init(CD3DXSKINMESH_INIT* si, const char* fileName)
{
	m_hWnd = si->hWnd;
	m_pDevice11 = si->pDevice;
	m_pContext11 = si->pDeviceContext;
...
HRESULT CDX9SkinMesh::CreateDeviceDx9(HWND hWnd)
{
	...
	if (pD3d9 == nullptr)
	{
		_ASSERT_EXPR(!(pD3d9 == nullptr), "Direct3D9の作成に失敗しました");
		return E_FAIL;
	}
	...
	if (FAILED(pD3d9->CreateDevice(
		...
		D3DCREATE_HARDWARE_VERTEXPROCESSING, &d3dpp, &m_pDevice9)))
	{
		if (FAILED(pD3d9->CreateDevice(
			D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_hWnd,
		...
		if (pAppMesh->pMaterial[i].szTextureName[0] != 0
			&& FAILED(
				D3DX11CreateShaderResourceViewFromFileA(
					m_pDevice11, pAppMesh->pMaterial[i].szTextureName,
	...
	for (DWORD i = 0; i < pMesh->dwNumMaterial; i++)
	{
		...
		if (pMesh->dwNumMaterial == 1) {
			...
			if (m_IsEnabelChangeTexttureSystem == 1) {
				...
				//別のテクスチャを設定二つ目.
				m_pContext11->PSSetShaderResources(0, 1, &m_pChangeTextures->pTextureTest);
			}
			else {
				//デフォルトテクスチャを設定.
				m_pContext11->PSSetShaderResources(0, 1, &pMesh->pMaterial[i].pTexture);
		...
		else
		{
			...
			if (m_B_IChangeTexttureSystem[i] == 1) {
				...
				if (change_flag == true)
				{
					...
					{
						//別のテクスチャを設定.
						m_pContext11->PSSetShaderResources(0, 1, &m_pBChangeTextures[i]->pTexture);
				...
				//別のテクスチャを設定二つ目.
				m_pContext11->PSSetShaderResources(0, 1, &m_pBChangeTextures[i]->pTextureTest);
			}
			else {
				//デフォルトテクスチャを設定.
				m_pContext11->PSSetShaderResources(0, 1, &pMesh->pMaterial[i].pTexture);
